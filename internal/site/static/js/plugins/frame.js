...
(() => {
    ...
    window.addEventListener("message", async (event) => {
        ...
        if (input.type !== "kumbuka-plugin-render" ||
            ...
            return;
        started = true;
        const send = (type, height = 0) => parent.postMessage({ type, token: input.token, height }, "*");
        try {
            ...
            const module = globalThis.kumbukaPlugin;
            ...
            const measure = () => send("kumbuka-plugin-ready", Math.min(10000, Math.max(24, Math.ceil(Math.max(root.getBoundingClientRect().height, root.scrollHeight)))));
            measure();
