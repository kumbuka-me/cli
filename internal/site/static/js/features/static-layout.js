...
function initNavigationTree() {
    ...
    if (!navigation)
        return;
    const nodes = [
        ...navigation.querySelectorAll("details[data-nav-node]"),
    ];
    navigation
        .querySelector("[data-navigation-collapse-all]")
        ?.addEventListener("click", () => {
        for (const node of nodes)
            node.open = false;
    });
    navigation
        .querySelector("[data-navigation-expand-all]")
        ?.addEventListener("click", () => {
        for (const node of nodes)
            node.open = true;
    });
...
function initSidebarVisibility() {
    const sidebar = document.querySelector("[data-sidebar]");
    const toggle = document.querySelector("[data-sidebar-visibility-toggle]");
    if (!sidebar || !toggle)
        return;
    ...
    function sync() {
        if (mobile.matches) {
            ...
            try {
                document.documentElement.classList.toggle("sidebar-hidden", window.localStorage.getItem(SIDEBAR_HIDDEN_KEY) === "true");
            }
            catch {
                // Storage may be unavailable in restricted browser contexts.
            }
        }
        const hidden = !mobile.matches &&
...
export function initStaticLayout() {
    initMobileSidebar();
