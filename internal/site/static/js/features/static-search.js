...
import { isRecord, requireArrayOf } from "../core/guards.js";
import { requestJSON } from "../core/http.js";
function isStaticSearchEntry(value) {
...
function resultLink(entry, className) {
...
async function renderSearchPage(container) {
    ...
    if (title)
        title.textContent = query ? `Results for “${query}”` : "All pages";
...
export function initStaticSearch() {
    for (const form of document.querySelectorAll("[data-static-search]")) {
        initSuggestions(form);
    }
    const pageResults = document.querySelector("[data-static-search-results]");
    if (pageResults)
        void renderSearchPage(pageResults);
}
